...
# nazwy <- c('_red_jaccard','_red_qgram',
#            '_red_lcs_lcs','_red_dl_dl','_red_jaccard_jaccard','_red_qgram_qgram')
...
# for(i in 1:5){
  ...
  for(a in nazwy){
    for(bb in c(5000, 10000, 35000, 70000))
      b <- c(b,sprintf("python /home/natalia/Text-clustering-basing-on-string-metrics/Python/python_mini_batch/results.py --i %d --typ %s --b %d; ", i, a, bb))
  }
  t[[i]] <- b
#}
