...
Mając daną sumę $\widetilde{W} = \widetilde{W}(\mathbf{m}) = \frac{1}{2}\sum\limits_{i=1}^{n} \norm{\mathbf{x}_i - \mathbf{m}_{C(i)}}_2^2 $, możemy policzyć gradient z $\mathbf{m}$: %szukamy minimum, licząc pochodną po $\mathbf{m}$ i przyrównując ją do zera:
\begin{equation}
\Delta \mathbf{m} = \frac{\partial \widetilde{W}(\mathbf{m})}{\partial \mathbf{m}} = 0 \Rightarrow
\Delta \mathbf{m} = \sum\limits_{i=1}^{n}\left\{
\begin{array}{l l}     
    \frac{1}{n_l}(\mathbf{x}_i - \mathbf{m}_l), & \text{gdy } l = C(i) \\
    0, & \text{wpp.}
\end{array}\right.
\end{equation}

Stąd dostajemy, że ... [TO DO!!!]
