...
Wstępną obróbkę danych przeprowadzono uwzględniając powyższe aspekty. Początkowo z części tekstowej usunięto wszystkie znaki nie będące literami alfabetu łacińskiego. Poddano jej dalszej obróbce po przetworzeniu części technicznej. Słowa, które występują w tekście, a pod którymi znajduje się link, zostawiono bez zmian. Wzory oraz kody usunięto w całości ze względu na trudność w rozróżnieniu czego podany fragment dotyczy oraz ze względu na krótkie znaki, które zawierały. Cytaty pozostawiono bez zmian. Przypisy usunięto z artykułów, jako że zawierają jedynie dodatkową treść. Źródła bibliograficzne usunięto w całości. Teksty oczyszczono także z tabel, rysunków, podpisów pod nimi oraz spisu treści. Sekcję ,,Zobacz też'' oraz kategorie pozostawiono jako potencjalne źródło podobnej tematyki do tej zawartej w tekście. Wszelkie znaczniki HTML-owe usunięto jako bezwartościowe z punktu widzenia treści artykułu. Podobnie teksty oczyszczono z tytułów, które pojawiały się w większości tekstów, tj. \emph{Zobacz też, Linki zewnętrzne} oraz \emph{Bibliografia}.
...
Tak otrzymane teksty podzielono na słowa, które przekształcono do wyrazów o małych literach. Do każdego tekstu dodano informacje o tym, jakie słowa i w jakich licznościach w nim wystąpiły. W ten sposób otrzymano $2\ 806\ 765$ różnych słów we wszystkich, tj. w $1\ 075\ 568$, artykułach. $49\%$ słów występuje tylko w jednym tekście (p. rys.~\ref{plot:001}). Nieco ponad $3.5\%$ słów wystąpiło w stu i więcej artykułach, natomiast jedynie $0.6\%$ wszystkich wyrazów pojawiło się w więcej niż tysiącu tekstów.
...
$44\%$ słów wystąpiło dokładnie raz we wszystkich artykułach (p. rys.~\ref{plot:002}). Prawie $4.3\%$ wyrazów pojawiło się ponad sto razy, natomiast mniej niż jeden procent słów wystąpiło tysiąc i więcej razy.
...
Te i podobne słowa oraz wyrazy jedno- i dwuznakowe, należy usunąć ze zbioru danych, gdyż nie wnoszą żadnej istotnej informacji o tematyce tekstu. Uczyniono to w kolejnym etapie wstępnej obróbki tekstów. Łącznie ze zbioru słów usunięto $1\ 822$ wyrazy.
...
Mediana długości słów wynosi $9$, średnia jest nieco wyższa. Najkrótsze występujące słowa są jednoznakowe, jest ich $249$. Najdłuższe słowo ma $128$ znaków. Słowa o długości ponad $11$ znaków stanowią $19.5\%$ wszystkich słów.
...
\section{Jakiś mądry tytuł / Utworzenie skupień ,,podobnych'' słów}

Po wstępnej obróbce danych uzyskano $2\ 804\ 943$ unikalnych słów, z czego blisko połowa wystąpiła jedynie raz we wszystkich tekstach. Algorytm dzielący teksty tematycznie bierze pod uwagę liczności słów, które występują w artykule a daną wejściową jest macierz o wymiarach liczba artykułów$\times$liczba słów. Jeśliby nie przetworzyć dalej danych, to macierz ta miałaby wymiary $1\ 075\ 568\ \times\ 2\ 804\ 943$, gdzie przeważająca większość rekordów byłaby równa zeru. Algorytm mógłby nie poradzić sobie z tak dużą ilością danych, zwłaszcza gdy większa część rekordów jest ,,niewypełniona''. Stąd zachodzi potrzeba zmniejszenia wymiaru danych. 

Ponieważ rekordy składają się ze słów, można wyznaczyć skupienia wyrazów ,,podobnych'' do siebie. Dalej dany tekst można przedstawić jako sumę liczby wystąpień słów z danego skupienia, zamiast liczby wystąpień pojedynczych słów. Przykładowo, jeśli skupienie $A$ składa się ze słów $s,\ t,\ u$, które wystąpiły w danym artykule, odpowiednio, $x,\ y,\ z$ razy, to słowa ze skupienia $A$ wystąpiły w tym tekście łącznie $x+y+z$ razy. Dzieląc wszystkie wyrazy na skupienia, możemy znacząco zmniejszyć liczbę danych, biorąc pod uwagę liczności grup słów, zamiast liczności pojedynczych wyrazów.
