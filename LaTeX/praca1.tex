...
d(s,t,0) &= \infty, \text{ gdy } |s| + |t| \\
d(\varepsilon,\varepsilon,0) &= 0
\end{align*}

Najprostszą metrykę liczącą odległość między napisami otrzymuje się poprzez wypisanie unikalnych $q$-gramów w obu napisach i porównanie które są wspólne. Jeśli przez $\mathcal{Q}(s,q)$ oznaczymy zbiór unikalnych $q$-gramów występujących w napisie $s$, to możemy zdefiniować odległość Jaccarda~\cite{Loo2014:stringdist}:
...
\begin{example}
Odległość Jaccarda między napisami \verb|palce| i \verb|pałace| dla $q = 2$ wynosi: $d_{jaccard}(\verb|palce|, \verb|pałace|, 2) \approx 0.714$, bo $\mathcal{Q}(\verb|palce|, 2) = \{\verb|pa|, \verb|al|, \verb|lc|, \verb|ce|\}$, a $\mathcal{Q}(\verb|pałace|, 2) = \{\verb|pa|, \verb|ał|, \verb|ła|, \verb|ac|, \verb|ce|\}$, więc odległość wynosi $1 - \frac{2}{7} \approx 0.714$.
\end{example}.
...
\begin{definition}
Niech $s = s_1 s_2 \ldots s_n$ będzie napisem z $\Sigma^*$ i niech $x \in \Sigma^q$ będzie $q$-gramem. Jeśli $s_i s_{i+1} \ldots s_{i+q-1} = x$ dla pewnego $i$, to $x$ \emph{wystąpiło} w $s$. Niech $v(s,x)$ oznacza liczbę wystąpień $x$ w $s$.
\end{definition} 
