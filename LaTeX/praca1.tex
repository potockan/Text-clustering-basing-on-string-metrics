...
\begin{equation}
\label{eq:013}
W = \sum\limits_{l=1}^{k}\sum\limits_{C(i) = k} d(\mathbf{x}_i, \mathbf{m}_l)n_l
\end{equation}

Średnie $\mathbf{m}_l$, $l = 1,\ldots, k$ nazywamy \emph{środkami skupień}. Równanie~\ref{eq:013} można uprościć do następującej postaci, która w praktyce jest łątwa w optymalizacji:
\begin{equation}
\label{eq:014}
\widetilde{W} = \sum\limits_{l=1}^{k}\sum\limits_{C(i) = k} d(\mathbf{x}_i, \mathbf{m}_l) = \sum\limits_{l=1}^{k}d(\mathbf{x}_i, \mathbf{m}_{C(i)})
\end{equation}

Algorytmy, które rozwiązują problem minimalizacji sumy~\ref{eq:014}, znane są pod nazwą \emph{metody $k$-średnich}.

\section{Algorytm wsadowy}
