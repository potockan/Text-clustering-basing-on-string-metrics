...
def clustering1(my_sparse_data, true_k, b):
    km = MiniBatchKMeans(n_clusters=true_k, init='k-means++', n_init=3, batch_size=b, max_no_improvement = 1000, max_iter = 100)
    km.fit(my_sparse_data)
    np.savetxt(os.getcwd() + "/wyniki_%d_%s.txt" % (b, typ), km.labels_, delimiter = ', ')
    return(km)
...
for b in [5000, 10000, 35000, 70000]:
    ...
    thefile = os.getcwd() + "/wyn_%d_%s" % (b, typ)
