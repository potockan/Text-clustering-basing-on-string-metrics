...
words_to_analize <- readLines("../Data/RObjects/words_to_analize.txt")
...
words <- readRDS("../Data/RObjects/words_111.rds")
...
word_order2 <- function(words_to_analize, words, method = 'lcs', q = 3, n = 5000){
  ...
  for(i in 1:(length(x)-1)){
    ...
    used[x[i]:(x[i+1]-1)] <- apply(
      stringdistmatrix(words$word, 
                       words_to_analize[x[i]:(x[i+1]-1)], 
                       method = method, 
                       q = q),
      2, which.min)
  }
...
saveRDS(used_dl, "../Data/RObjects/used_dl.rds")
...
# }))
# saveRDS(used_jaccard, "../Data/RObjects/used_jaccard.rds")
...
# saveRDS(used_qgram, "../Data/RObjects/used_qgram.rds")
