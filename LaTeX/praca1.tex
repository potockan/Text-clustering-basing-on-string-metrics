...
\subsection{Jednorodność, zupełność oraz miara $\textrm{V}$}
...
Aby zdefiniować jednorodność i zupełność, potrzebujemy pojęć \emph{entropii} i \emph{warunkowej entropii}:
\begin{align*}
H(C) & = - \sum\limits_{c = 1}^{|C|}\frac{n_c}{n}\cdot \log{\frac{n_c}{n}},  \\
H(C|K) & = - \sum\limits_{c = 1}^{|C|}\sum\limits_{k = 1}^{|K|}\frac{n_{c,k}}{n}\cdot \log{\frac{n_{c,k}}{n}},
\end{align*}
