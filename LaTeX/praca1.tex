...
Metoda ta może być zdefiniowana i zaimplementowana przy pomocy rekurencyjnych wzorów Lance'a-Williamsa. Wzory te to nieskończona rodzina algorytmów grupowania aglomeracyjnego, które są reprezentowane przez rekurencyjny wzór aktualizujący odległości skupień na każdym poziomie. W każdym kroku niezbędne jest, aby zoptymalizować funkcję celu. Rekurencyjna formuła ułatwia więc znalezienie odpowiedniej pary skupień.	
...
\begin{definition}
Wzory Lance'a-Williamsa definiujemy następująco:
$$
...
$$
D_{(ij)l} = \frac{n_i+n_l}{n_i+n_j+n_l}_i D_{il} + \frac{n_j+n_l}{n_i+n_j+n_l} D_{jl} - \frac{n_l}{n_i+n_j+n_l} D_{ij},
$$
